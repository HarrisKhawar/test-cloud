...
const createCreditAppAgreement = (partner, signature) => {
  return new Promise(async (resolve, reject) => {
    try {
      // Get Path static path to /assets/docs/vehicle-rental-agreement.pdf
      const path = `${__dirname}/../../assets/docs/credit-app-agreement.pdf`;
      ...
      const file = bucket.file(`partners/${partner.id}/credit-app-agreement.pdf`);
...
const addPartnerInfo = (pdfDoc, partner) => {
  ...
  y = y - 15;
  lastPage.drawText(partner?.drivers_license_number || "-", {
    x: 200,
    y: y,
    size: 11,
  });
  y = y - 15;
  lastPage.drawText(partner?.ssn || "-", {
    x: 200,
    y: y,
    size: 11,
  });
...
module.exports = { createCreditAppAgreement };
