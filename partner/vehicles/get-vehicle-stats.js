...
* Request Body:
    ...
    - year: <string>
/* ========================================================================== */
const get_vehicle_stats = async (request, response) => {
  try {
    // Handle Request
    ...
    payments.forEach((payment) => {
      ...
      if (!vehicleId) return;
      if (!payment.description || !payment.description.includes("Booking")) return;

      // Check if payment is from this year
      if (
        payment.date_created.toDate().getFullYear() !== Number(year)
      )
        return;
    ...
    logSuccess("partner_get_vehicle_stats", "Successfully fetched vehicle stats");
    ...
    logError("partner_get_vehicle_stats", err.message);
    response.status(500).send("Error getting vehicle stats");
  }
};

module.exports = get_vehicle_stats;
